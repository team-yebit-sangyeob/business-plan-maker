...
대화로 사업 아이디어를 받아 **10개 슬롯**(문제·타겟·솔루션·시장·차별점·수익모델·목표·리소스·일정·리스크)을 하나씩 채워 계획서로 만든다. 매 턴:

1. **분해·분류** — 한 발화를 의미 단위로 나누고 유형(주장·질문·정정·도구질문 등)을 매긴다.
2. **검증** — 검증할 주장이면 웹 리서치(외부 사실)·회사 문서 RAG(내부 정합성)·논리 검증을 병렬로 거친다.
3. **대화** — 결과를 한 응답으로 엮고 다음 빈 슬롯을 묻는다.

필수 슬롯(문제·타겟·목표)이 차면 계획서를 출력한다. 설계 원칙은 **"비싸거나 되돌리기 어려운 결정(워커 라우팅·슬롯 쓰기)은 코드가 결정론으로, 표현은 LLM이"** — 자세한 흐름은 `agents/orchestrator/ORCHESTRATOR.md`.
...
/web             Vite + React 프론트
/docs            코드 워크스루·스펙 문서 (gitignore된 로컬 — 클론엔 없음)
```
