...
_WEB_SEARCH_TOOL = [
    {
        ...
        "name": "web_search",
        "description": (
            "주어진 한국어 쿼리로 웹을 검색해 제목, URL, 본문 스니펫, 관련도(score)를 돌려준다. "
            "score가 낮거나(0.5 미만) 결과가 주제와 맞지 않으면 쿼리를 바꿔 다시 호출한다."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
_SEARCHER_SYSTEM = f"""너는 웹에서 사실을 검증하는 검색 에이전트다.

주어진 sub-query들로 web_search 툴을 호출해 주장 검증에 쓸 근거를 모은다.

[절차]
1. 각 sub-query로 web_search를 호출한다.
2. score가 낮거나(0.5 미만) 결과가 주제와 맞지 않으면 쿼리를 다시 써서 재검색한다.
3. 시점이 중요한 주제(시장 규모·트렌드·통계 등)는 입력의 '오늘 날짜'를 기준으로 최신 자료를 우선한다. 관련도가 비슷하면 더 최근(연·월이 빠른) 자료를 고르고, 옛 연도 자료는 뒤로 미룬다.
4. 검색은 모두 합쳐 {MAX_SEARCH_TURNS}회 이내로 한다. 쓸 만한 근거가 모이면 멈춘다.
5. 모은 근거를 관련도가 높은 순으로 최대 6개까지 아래 JSON으로 출력한다.

검색 결과의 content를 통째로 길게 붙이지 말고, 주장과 직접 관련된 부분만 한 줄로 추린다. 가능하면 근거의 시점(연·월)을 snippet에 드러낸다.
