...
지원 intent(conversation_spec TRIGGER MATRIX 전체):
  ...
  reason_over_context — 추론·도출·종합: 이미 모은 근거(session_evidence)와 대화 이력에서 직접 추론해 답(워커·검색 없이). 사용자가 슬롯 값을 어시스턴트 판단으로 청하면 되묻지 말고 구체적 후보를 제안한다.
